...
const humanizeMembers = async (members) => {
  return (
    await Promise.allSettled(members.map((member) => getUserInfo(member)))
  ).map((member) => member.value.user.real_name);
};
...
const humanizePreviousMatches = async (previousMatches) => {
  ...
  for (root in previousMatches) {
    ...
    const temp = await Promise.allSettled(
      previousMatches[root].map((previous) => getUserInfo(previous))
    );
...
const humanizeCurrentMatches = async (currentMatches) => {
  ...
  for (const match of currentMatches) {
    const temp = await Promise.allSettled(
      match.map((person) => getUserInfo(person))
    );
...
const validPairing = (member, potentialPartner, previousMatches) => {
  ...
  return (
    potentialPartner !== member &&
    !previousMatches[member]?.includes(potentialPartner)
  );
};
...
const getMatches = async (members, previousMatches) => {
  ...
  if (members.length % 2) {
    ...
    filteredMembers = members.filter(
      (member, index) => index !== indexToIgnore
    );
  }
  ...
  filteredMembers.forEach((member) => {
    ...
    if (!partnerFound) {
      const potentials = filteredMembers.filter(
        (potential) => potential !== member && !partnered.has(potential)
      );
...
const processMatch = async (match, previousMatches) => {
  match.forEach(async (root) => {
    match.forEach(async (other) => {
      ...
      if (!previousMatches[root].includes(other))
        previousMatches[root].push(other);
    });
  });
};

// Processes the current matches and updates previous matches
const updateCurrentAndPreviousMatches = async (
  currentMatches,
  previousMatches
) => {
  for (const match of currentMatches) {
...
const createGroupChats = async (currentMatches) => {
  for (const match of currentMatches) {
    ...
    const newGroupId = (await Bot.client.conversations.open({ users })).channel
      .id;
